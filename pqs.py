...
def DNLS(x, P, backend=math):
     ...
     P=np.array(P) 
     N=len(x)-1
     func= lambda x, P:[ (P[0] -P[3])*x[n]+P[1]*(x[n-1]+x[n+1])+P[2]*x[n]*(2*np.absolute(x[n])**2 + np.absolute(x[n-1])**2  + np.absolute(x[n+1])**2 )   
                            for n in  range(len(x)) if (n!=0  and n!=len(x) -1) ] 
     result=np.array(func(x, P))
     result=np.append(result , (P[0] -P[3])*x[N]+P[1]*(x[N-1])+P[2]*x[N]*(2*np.absolute(x[N])**2 + np.absolute(x[N-1])**2 ) )
     result= np.insert(result ,  0 ,  (P[0] -P[3])*x[0]+P[1]*(x[1])+P[2]*x[0]*(2*np.absolute(x[0])**2 + np.absolute(x[1])**2 ) )
     return result
...
def matrices(q, P):
    ...
    for n in range(1, N):
        for m in range(len(q)):
           A[n][m] =(-lam +chi*(np.absolute(q[n+1])**2 +np.absolute(q[n-1])**2+ 4*np.absolute(q[n])**2 -2*chi*q[n]**2) )*kron(n,m)+ (V +chi*q[n]*( conj(q[n+1])-q[n+1]))*kron(n,m-1)  + (V +chi*q[n]*( conj(q[n-1])-q[n-1]))*kron(n,m+1) 
           B[n][m] = (lam -chi*( np.absolute(q[n+1])**2 +np.absolute(q[n-1])**2 + 4*np.absolute(q[n])**2 - 2*chi*q[n]**2) )*kron(n,m)- (V +chi*q[n]*( q[n+1]+q[n+1]))*kron(n,m-1) - (V +chi*q[n]*( q[n-1] + q[n-1]))*kron(n,m+1)                   
    ...
    A[0][0]= (-lam +chi*( np.absolute(q[1])**2+ 4*np.absolute(q[0])**2 -2*chi*q[0]**2) ) 
    A[N][N]= (-lam +chi*( np.absolute(q[N-1])**2+ 4*np.absolute(q[N])**2 -2*chi*q[N]**2) ) 
    B[0][0]= (lam -chi*( np.absolute(q[1])**2  + 4*np.absolute(q[0])**2 -2*chi*q[0]**2) ) 
    B[N][N]= (lam -chi*( np.absolute(q[N-1])**2+ 4*np.absolute(q[N])**2 -2*chi*q[N]**2) )
